...
class ContributionStore:
        ...
        *,
        ...
        record = {
            ...
            "properties": {
                ...
                # Stored so the UI can restore the polygon as a candidate
                # after a delete. Only meaningful when source='overture'.
    ...
    def add_many(
        ...
        *,
        source: str = "overture",
        author: Optional[str] = None,
        edited_flags: Optional[List[bool]] = None,
        original_ids: Optional[List[Optional[str]]] = None,
        original_geometries: Optional[List[Optional[dict]]] = None,
        notes: Optional[str] = None,
    ) -> dict:
        """Bulk-save ``features``.

        Each feature is validated independently; failures don't block the
        rest. Returns ``{"saved": [Feature, ...], "errors": [{index, message}, ...]}``.
        """
        saved: List[dict] = []
        errors: List[dict] = []
        for i, feature in enumerate(features):
            try:
                rec = self.add(
                    ...
                    author=author,
                    source=source,
                    original_id=(original_ids[i] if original_ids else None),
                    edited=bool(edited_flags[i]) if edited_flags else False,
                    original_geometry=(
                        original_geometries[i] if original_geometries else None
                    ),
                )
                saved.append(rec)
            except ContributionError as e:
                errors.append({"index": i, "message": str(e)})
            except Exception as e:  # defensive — should be caught above
                errors.append({"index": i, "message": f"unexpected: {e}"})
    ...
    def delete_many(self, contribution_ids: List[str]) -> dict:
        """Delete each id in ``contribution_ids``.

        Returns ``{"deleted_ids": [...], "missing_ids": [...]}`` so the
        caller can distinguish between "didn't exist" and "actually
        removed".
        """
        deleted: List[str] = []
        missing: List[str] = []
    ...
    def delete_all(self) -> int:
        """Remove every contribution file. Returns the count removed."""
        with self._lock:
            removed = 0
...
def overture_only_candidates(
    ...
    *,
    ...
    Returns a tuple ``(candidates, meta)`` where ``meta`` carries
    ``{"osm_available": bool, "osm_error": str | None, "overture_count": int}``
    so the API layer can warn the user when OSM Overpass was unreachable
    (in that case every Overture polygon comes back as a candidate since
    we can't filter against OSM).
    """
    ...
    meta: dict = {"osm_available": True, "osm_error": None, "overture_count": 0}

    gdf = _fetch_and_filter_buildings(lat, lon, radius_meters)
    if gdf is None or len(gdf) == 0:
        return [], meta
    meta["overture_count"] = int(len(gdf))

    try:
        osm_polys = get_osm_building_polygons(
            lat, lon, radius_meters,
            all_buildings=osm_all_buildings,
            bypass_cache=bypass_cache,
        )
    except Exception as e:
        # Overpass is flaky / rate-limited / down — don't take the whole
        # detection down with it. Fall through with osm_union=None so
        # every Overture polygon becomes a candidate; flag that the OSM
        # filter wasn't applied.
        osm_polys = []
        meta["osm_available"] = False
        meta["osm_error"] = f"{type(e).__name__}: {e}"
